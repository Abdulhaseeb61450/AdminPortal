...
public class LogOutTimerUtil {
    ...
    static final int LOGOUT_TIME = 900000; // delay in milliseconds i.e. 5 min = 300000 ms or use timeout argument
    ...
    static class ForegroundCheckTask extends AsyncTask<Context, Void, Boolean> {
        ...
        private boolean isAppOnForeground(Context context) {
            ...
            for (ActivityManager.RunningAppProcessInfo appProcess : appProcesses) {
                if (appProcess.importance == ActivityManager.RunningAppProcessInfo.IMPORTANCE_CACHED && appProcess.processName.equals(packageName)) {
                    return true;
                }
                else if (appProcess.importance == ActivityManager.RunningAppProcessInfo.IMPORTANCE_TOP_SLEEPING && appProcess.processName.equals(packageName))
                {
                    return true;
                }
